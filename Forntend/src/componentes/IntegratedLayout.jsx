...
const IntegratedLayout = ({ children }) => {
  ...
  const menuItems = [
    { to: "/administrador/admindashboard", icon: RiDashboardLine, label: "Dashboard", color: "from-blue-500 to-blue-600" },
    { to: "/administrador/perfil", icon: RiUser3Line, label: "Mi Perfil", color: "from-purple-500 to-purple-600" },
    { to: "/administrador/usuario", icon: RiTeamLine, label: "Usuarios", color: "from-green-500 to-green-600" },
    { to: "/administrador/Horario", icon: RiTimeLine, label: "Horarios", color: "from-orange-500 to-orange-600" },
    { to: "/administrador/Rutina", icon: RiFileList3Line, label: "Rutinas", color: "from-pink-500 to-pink-600" },
    { to: "/administrador/Equipo", icon: RiArchiveLine, label: "Equipos", color: "from-indigo-500 to-indigo-600" },
    { to: "/administrador/Reserva", icon: RiBookmarkLine, label: "Reservas", color: "from-cyan-500 to-cyan-600" },
    { to: "/administrador/Reporte", icon: RiBarChart2Line, label: "Reportes", color: "from-red-500 to-red-600" },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black">
      ...
      `}>
        {/* Header del sidebar con logo - ALINEADO A LA IZQUIERDA CON LOGO REDONDO Y GRANDE */}
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1 h-[calc(100vh-160px)]">
          ...
          {menuItems.slice(0, 3).map(({ to, icon: Icon, label, color }) => (
            <Link
              key={label}
              to={to}
          ...
          {/* Separador y sección General */}
          {!isCollapsed && (
            <div className="my-4">
              <div className="h-px bg-gray-700/50 mx-3 mb-3"></div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-3 mb-3">
                General
              </h3>
            </div>
          ...
          {menuItems.slice(3).map(({ to, icon: Icon, label, color }) => (
            <Link
              key={label}
              to={to}
              className={`group flex items-center ${isCollapsed ? 'justify-center p-3' : 'gap-3 px-3 py-2.5'} rounded-xl 
      ...
      {isMobile && (
        <>
          ...
          `}>
            {/* Header móvil con logo redondo y grande */}
            <div className="h-20 flex items-center px-4 border-b border-gray-700/50">
              <div 
                className="flex items-center gap-3 cursor-pointer group"
                onClick={handleLogoClick}
