...
// Configuraciones para cada tipo de cliente
const CLIENT_CONFIGS = {
  powerplate: {
    menuItems: [
      ...
      {
        to: "/cliente/HorarioTurnoCliente",
        icon: RiListCheck2,
        label: "Horario Turnos",
        color: "from-teal-500 to-teal-600",
      },
      {
        to: "/cliente/ReservaCliente",
        icon: RiBookmarkLine,
        label: "Reservas",
...
const IntegratedLayoutCliente = ({ children }) => {
  ...
  // Determinar el tipo de cliente automáticamente
  const getClientType = () => {
    if (!user) return "calistenia";

    // Campo categoria (el más importante)
    if (user.categoria) {
    ...
    // Estrategia 2: Campo directo tipo_cliente
    if (user.tipo_cliente) {
      const tipoCliente = user.tipo_cliente.toLowerCase();
      if (tipoCliente === "powerplate" || tipoCliente === "power-plate") {
        return "powerplate";
      }
    }
  ...
  const config = CLIENT_CONFIGS[clientType] || CLIENT_CONFIGS.calistenia;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black">
      ...
      `}>
        ...
        {/* Indicador de tipo de cliente en sidebar expandido */}
        {!isCollapsed && (
          <div className="px-3 py-3 border-b border-gray-700/30">
            <div className="bg-gradient-to-r from-yellow-400/10 to-yellow-500/10 border border-yellow-400/30 rounded-lg px-3 py-2">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1 h-[calc(100vh-160px)]">
          ...
          {config.menuItems.map(({ to, icon: Icon, label, color }) => (
            <Link
              key={label}
              to={to}
      ...
      {isMobile && (
        <>
          ...
          `}>
            ...
            {/* Indicador de tipo de cliente móvil */}
...
export default IntegratedLayoutCliente;