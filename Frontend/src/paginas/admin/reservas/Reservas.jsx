...
const Reservas = () => {
  ...
  const handleMarcarAsistenciaDirecta = async (reserva) => {
    try {
      ...
      if (reserva.asistencia && reserva.asistencia > 0) {
        showNotification("warning", "Esta reserva ya tiene asistencia marcada");
        return;
      }
      ...
      const resultado = await ReservaService.marcarAsistencia(
        reserva.id_reserva,
        100,
      ...
      if (resultado && resultado.success === false) {
        ...
        // Recargar las reservas para ver el cambio
  ...
  const reservasFiltradas = reservas.filter((reserva) => {
    const cumpleFiltros = {
      ...
      entrenador:
        !filtros.entrenador ||
        reserva.entrenador_id.toString() === filtros.entrenador,
      busqueda:
        !filtros.busqueda ||
  ...
  const permisos = usuario
    ? {
        ...
        puedeCancelar: (reserva) =>
          ReservaService.puedesCancelarReserva(
            usuario.rol,
            reserva,
            usuario.id_usuario
          ),
        puedeMarcarAsistencia: (reserva) =>
          ReservaService.puedesMarcarAsistencia(
            usuario.rol,
            reserva,
            usuario.id_usuario
          ),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="confirmada" className="bg-gray-700">
                  Confirmada
                </option>
                <option value="cancelada" className="bg-gray-700">
                  Cancelada
                </option>
              </select>
            </div>
            ...
            {usuario.rol === "administrador" && entrenadores.length > 0 && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFiltroChange("entrenador", e.target.value)
                  }
                  className="w-full bg-gray-700 border border-gray-600 text-gray-100 rounded-lg px-4 py-3 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition-all"
                >
                  <option value="" className="bg-gray-700">
