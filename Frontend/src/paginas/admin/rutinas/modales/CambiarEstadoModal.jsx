...
const CambiarEstadoModal = ({ isOpen, onClose, onConfirm, rutina, error }) => {
  ...
  const getMusculosUnicos = () => {
    ...
    const musculos = [...new Set(rutina.partes_musculo)];
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-start justify-center z-50 backdrop-blur-sm p-4 pt-4">
      <div className="bg-gray-900 border-2 border-yellow-500/30 rounded-xl p-6 w-full max-w-4xl shadow-2xl shadow-yellow-500/20 max-h-[95vh] overflow-y-auto">
        <style>
          {`
                ...
                {rutina.nombres_ejercicios.map((nombre, index) => (
                  <div key={index} className="bg-gray-700/50 rounded-lg p-3 border border-gray-600">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h4 className="font-medium text-gray-100 text-sm mb-1 capitalize">{nombre}</h4>
